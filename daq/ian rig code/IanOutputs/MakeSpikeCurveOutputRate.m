...
P1= [.075];%[0.05 .1 .15];%List of Powers To Use
S1 = [1 3 5 10 20 30]; %spikes to Add (@ 30Hz)
R1 =  [5 10 50]; %rates to probe (@ 5 spikes)
...
for i=1:numel(R1)
    count=count+1;
    powerList(count)       = { P1 };
    waitList(count)        = [ 6 ]; %time after stim before next stim ms
    hzList(count)          = R1(i);
    pulseList(count)       = 5; %previously 10; 5 is standard as of 5/15/19
    holosPerCycle(count)   = [ 1 ]; %groups to interleave
    cellsPerHolo(count)    = [ cellsAtATime ];
    divTotalCells(count)   = [ 1 ]; %divide total number of holos
    holoSets(count)        = [ 1 ]; %unique groups of cells per
    setlinks(count)        = [ 1 ];
end
% for i=1:3
%     powerList{i}       = [P1];
